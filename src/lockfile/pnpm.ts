...
export const pnpmGenerator: LockFileGenerator = {
  generate(graph: DependencyGraph, rootPkg: RootPackageJson): string {
    ...
    const depSections: [string, Record<string, string> | undefined][] = [
  ...
  parse(content: string): DependencyGraph {
    ...
    function flushPkg(): void {
      ...
      graph.set(`node_modules/${name}`, {
        name,
        version,
        integrity: currentPkg.integrity as string | undefined,
        resolved: currentPkg.tarball as string | undefined,
        dev: currentPkg.dev === true || currentPkg.dev === "true",
        optional: currentPkg.optional === true || currentPkg.optional === "true",
        dependencies: currentPkg.dependencies as Record<string, string> | undefined,
        optionalDependencies: currentPkg.optionalDependencies as Record<string, string> | undefined,
      });
    ...
    for (const line of lines) {
      ...
      if (line.match(/^ {4}\S/) && currentPkgId) {
        ...
        if (resMatch) {
          ...
          if (tarMatch) currentPkg.tarball = tarMatch[1];
          inSubSection = "";
          continue;
        }
        ...
        if (trimmed === "dependencies:" || trimmed === "optionalDependencies:") {
          ...
          if (!currentPkg[inSubSection]) currentPkg[inSubSection] = {};
          continue;
        }
      ...
      if (line.match(/^ {6}\S/) && currentPkgId && inSubSection) {
        ...
        if (kvMatch) {
          const deps = currentPkg[inSubSection] as Record<string, string>;
...
function findByName(graph: DependencyGraph, name: string): { version: string } | undefined {
