...
export const bunGenerator: LockFileGenerator = {
  generate(graph: DependencyGraph, rootPkg: RootPackageJson): string {
    const workspaces: Record<string, Record<string, unknown>> = {};
    const packages: Record<string, unknown[]> = {};

    // Root workspace
    const rootWs: Record<string, unknown> = {};
    if (rootPkg.name) rootWs.name = rootPkg.name;
    if (rootPkg.dependencies) rootWs.dependencies = rootPkg.dependencies;
    if (rootPkg.devDependencies) rootWs.devDependencies = rootPkg.devDependencies;
    if (rootPkg.optionalDependencies) rootWs.optionalDependencies = rootPkg.optionalDependencies;
    workspaces[""] = rootWs;
...
export const bunParser: LockFileParser = {
  parse(content: string): DependencyGraph {
    ...
    for (const [name, value] of Object.entries(packages)) {
      ...
      const resolvedId = value[0] as string;
      ...
      const meta = typeof value[2] === "object" && value[2] !== null ? value[2] as Record<string, string> : undefined;
